...
# Machine Learning for the Quantified Self (Springer, Ch. 7)
# ------------------------------------------------------------ #
...
class ClassificationAlgorithms:
    # Forward selection for classification which selects a pre-defined number
    # of features (max_features) that show the best accuracy. We assume a decision
    # tree learning for this purpose, but this can easily be changed. It return
    # the best features.
    ...
    def feedforward_neural_network(
        ...
        if grid_search and print_model_details:
    ...
    def support_vector_machine_with_kernel(
        ...
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=svm.classes_
        )
        frame_prob_test_y = pd.DataFrame(
            pred_prob_test_y, columns=svm.classes_
        )
    ...
    def support_vector_machine_without_kernel(
        ...
        if grid_search:
            ...
            svm = GridSearchCV(
                LinearSVC(), tuned_parameters, cv=5, scoring="accuracy"
            )
        else:
        ...
        distance_training_platt = 1 / (
            1 + np.exp(svm.decision_function(train_X))
        )
        pred_prob_training_y = (
            distance_training_platt
            / distance_training_platt.sum(axis=1)[:, None]
        )
    ...
    def k_nearest_neighbor(
        ...
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=knn.classes_
        )
        frame_prob_test_y = pd.DataFrame(
            pred_prob_test_y, columns=knn.classes_
        )
    ...
    def decision_tree(
        ...
        d_tree.fit(train_X, train_y.values.ravel())

        if grid_search and print_model_details:
            print(d_tree.best_params_)

        if grid_search:
            d_tree = d_tree.best_estimator_

        # Apply the model
        pred_prob_training_y = d_tree.predict_proba(train_X)
        pred_prob_test_y = d_tree.predict_proba(test_X)
        pred_training_y = d_tree.predict(train_X)
        pred_test_y = d_tree.predict(test_X)
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=d_tree.classes_
        )
        frame_prob_test_y = pd.DataFrame(
            pred_prob_test_y, columns=d_tree.classes_
        )
        ...
        if print_model_details:
            ordered_indices = [
                i[0]
                for i in sorted(
                    enumerate(d_tree.feature_importances_),
                    key=lambda x: x[1],
                    reverse=True,
                )
            ]
            ...
            for i in range(0, len(d_tree.feature_importances_)):
                ...
                print(d_tree.feature_importances_[ordered_indices[i]])
            tree.export_graphviz(
                d_tree,
                out_file=export_tree_path + export_tree_name,
                feature_names=train_X.columns,
                class_names=d_tree.classes_,
            )
    ...
    def naive_bayes(self, train_X, train_y, test_X):
        ...
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=nb.classes_
        )
    ...
    def random_forest(
        ...
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=rf.classes_
        )
